...
static volatile int g_imu_mode = 0; /* 0=manual, 1=Phone IMU+PID */
static volatile float g_phone_pitch = 0.0f;
static volatile float g_phone_roll = 0.0f;
...
static float current_gripper = 90.0f;
static float current_pitch = 0.0f;
static float current_roll = 0.0f;

/* PID状态变量（供Web显示） */
static volatile float g_pid_pitch_error = 0.0f;
static volatile float g_pid_pitch_output = 0.0f;
 ...
 *   pitch→ 肩(ch1) : 90-pitch*0.5 ，pitch>0（前倾）→肩<90°（前伸）
 *   pitch→ 肘(ch2) : 90+pitch*0.8 ，pitch>0（前倾）→肘>90°（外扩）
 *   pitch→ 腕(ch3) : 90-pitch*1.2 ，腕关节幅度最大，保持末端水平
 *
...
{
    const float shoulder_ratio = 0.5f;
    const float elbow_ratio    = 0.8f;
    const float wrist_ratio    = 1.2f;

    /* 底座：roll直接叠加，clamp到[0, 180]，跳变>170° 判定为传感器翻转，丢弃 */
    static float s_prev_raw_base  = 90.0f;
    static float s_prev_raw_pitch = 0.0f;

    float raw_base = 90.0f + roll;
    float delta = raw_base - s_prev_raw_base;
    if (delta > 170.0f || delta < -170.0f) {
        raw_base = s_prev_raw_base;   /* 传感器翻转 artifact → 保持原位 */
    } else {
        s_prev_raw_base = raw_base;   /* 正常移动 → 更新记忆 */
    }

    float safe_pitch = pitch;
    delta = safe_pitch - s_prev_raw_pitch;
    if (delta > 170.0f || delta < -170.0f) {
        safe_pitch = s_prev_raw_pitch;
    } else {
        s_prev_raw_pitch = safe_pitch;
    }
    ...
    /* 肩关节：safe_pitch反向，clamp到[60, 120] */
    float shoulder = 90.0f - safe_pitch * shoulder_ratio;
    if (shoulder <  60.0f) shoulder =  60.0f;
    if (shoulder > 120.0f) shoulder = 120.0f;

    /* 肘关节：safe_pitch同向，clamp到[40, 140] */
    float elbow = 90.0f + safe_pitch * elbow_ratio;
    if (elbow <  40.0f) elbow =  40.0f;
    if (elbow > 140.0f) elbow = 140.0f;

    /* 腕关节：safe_pitch反向，幅度最大，clamp到[20, 160] */
    float wrist = 90.0f - safe_pitch * wrist_ratio;
    if (wrist <  20.0f) wrist =  20.0f;
    if (wrist > 160.0f) wrist = 160.0f;

    group->count = 4;
    group->commands[0].channel = 0;  group->commands[0].angle = base;
    group->commands[1].channel = 1;  group->commands[1].angle = shoulder;
    group->commands[2].channel = 2;  group->commands[2].angle = elbow;
    group->commands[3].channel = 3;  group->commands[3].angle = wrist;
}
    ...
    while (1)
    {
        ...
        if (g_imu_mode == 1)
        {
            ...
            if (imu_first_entry) {
                ...
                PID_Reset(&pid_pitch);
                filtered_base     = 90.0f + g_phone_roll;
                filtered_shoulder = 90.0f - g_phone_pitch * 0.5f;
                filtered_elbow    = 90.0f + g_phone_pitch * 0.8f;
                filtered_wrist    = 90.0f - g_phone_pitch * 1.2f;
                imu_first_entry = false;
            }

            map_IMU_To_Servos(g_phone_pitch, g_phone_roll, &target_group);
            ...
            if (feedback_ok)
            {
                ...
                /* 末端姿态误差 */
                float pitch_error = g_phone_pitch - feedback.pitch;
                ...
                /* 更新PID状态供Web显示 */
                g_pid_pitch_error = pitch_error;
                g_pid_pitch_output = pid_output;
            }
            ...
            if (target_group.commands[0].angle > last_base + threshold || target_group.commands[0].angle < last_base - threshold)
            {
                need_send = true;
            }
            if (target_group.commands[1].angle > last_shoulder + threshold || target_group.commands[1].angle < last_shoulder - threshold)
            {
                need_send = true;
            }
            if (target_group.commands[2].angle > last_elbow + threshold || target_group.commands[2].angle < last_elbow - threshold)
            {
                need_send = true;
            }
            if (target_group.commands[3].angle > last_wrist + threshold || target_group.commands[3].angle < last_wrist - threshold)
            {
                need_send = true;
            }
        ...
        else
        {
            ...
            {
                PID_Reset(&pid_pitch);
                g_pid_pitch_error = 0.0f;
                g_pid_pitch_output = 0.0f;
            }
        }
...
void Servo_Task(void *pvParameters)
{
    ...
    {
        /* 先获取 I2C 锁，再出队指令。
         * 这样如果锁被占用，队列不会丢指令，等锁释放后取出执行。
         * 用 20ms 超时避免锁长时间不可用时堵死队列。 */
        if (xSemaphoreTake(xI2CMutex, pdMS_TO_TICKS(20)) == pdTRUE)
        {
            /* 非阻塞读取：队列为空则立即释放锁，不占着锁空等 */
            if (xQueueReceive(xControlQueue, &group, 0) == pdTRUE)
            {
                for (uint8_t i = 0; i < group.count; i++)
                {
                    ...
                    {
                        case 0:
                            current_base = group.commands[i].angle;
                            break;
                        case 1:
                            current_shoulder = group.commands[i].angle;
                            break;
                        case 2:
                            current_elbow = group.commands[i].angle;
                            break;
                        case 3:
                            current_wrist = group.commands[i].angle;
                            break;
                        case 4:
                            current_gripper = group.commands[i].angle;
                            break;
                    }
                }
                /* 执行完后让出一点时间，让 IMU 任务有机会拿到锁 */
                xSemaphoreGive(xI2CMutex);
...
static void web_command_handler(const char *command, float value)
{
    ...
    else if (strcmp(command, "kinematic_move") == 0)
    {
        /* 无坐标参数：使用默认硬编码位置 */
        g_imu_mode = 0;
        EndEffectorPos start_pos = {80.0f, 0.0f, 200.0f};
        EndEffectorPos end_pos = {-60.0f, 40.0f, 220.0f};
...
void WebStatus_Task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        status.gripper_angle = current_gripper;
        status.mode = g_imu_mode;

        /* PID反馈数据 */
        status.pid_pitch_error = g_pid_pitch_error;
        status.pid_pitch_output = g_pid_pitch_output;

        status.target_pitch = g_phone_pitch;
